import type {
  CompareBasketItemDto,
  CompareBasketResponseDto,
  CompareBasketResultDto,
...
export type StoreId = (typeof STORES_DATA)[number]["id"];
export const getStoreIdByName = (apiName: string | null | undefined): StoreId | "other" => {
  if (!apiName) return "other";

  const clean = apiName.toUpperCase();
  const found = STORES_DATA.find((store) => clean.includes(store.name.toUpperCase()));
...
export const api = {
  search: async (query: string): Promise<ProductResult[]> => {
    ...
    try {
      ...
      return (await res.json()) as ProductSearchResultDto[];
    } catch (error) {
  ...
  compareBasket: async (items: CompareBasketItemDto[]): Promise<CompareBasketResultDto[]> => {
    try {
      ...
      const json = (await res.json()) as CompareBasketResponseDto;
