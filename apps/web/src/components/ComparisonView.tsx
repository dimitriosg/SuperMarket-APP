    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 pb-20">
      <div className="max-w-6xl mx-auto px-4 pt-8 space-y-10">
        <header className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black tracking-tight text-slate-900">Basket Analysis</h1>
            <p className="text-slate-500 mt-1 font-medium">
              Σύγκρινε τιμές σε πραγματικό χρόνο για {formattedBasketCount} προϊόντα.
            </p>
          </div>
          <div className="flex items-center gap-3">
            <label className="text-xs font-bold uppercase tracking-widest text-slate-400">Περιοχή</label>
            <select
              value={regionId}
          ...
          <div className="lg:col-span-8 space-y-6">
            <div className="grid gap-6 md:grid-cols-2">
              <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                <div className="text-[10px] uppercase tracking-widest font-black text-slate-400">Καλύτερο κατάστημα</div>
                {bestSingleStore ? (
                  <>
                    <div className="mt-2 text-2xl font-black text-slate-800">{bestSingleStore.storeName}</div>
                  ...
                  <p className="text-sm text-slate-400 mt-3">{EMPTY_TEXT}</p>
                )}
              </div>

              <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                <div className="text-[10px] uppercase tracking-widest font-black text-slate-400">Mix & Match</div>
                {bestMultiStore ? (
                  <>
                    <div className="mt-2 text-2xl font-black text-slate-800">{bestMultiStore.storeName}</div>
                    <div className="mt-4 text-3xl font-black text-emerald-500">
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-xl font-black text-slate-800">Λεπτομέρειες σύγκρισης</h2>
                  <p className="text-sm text-slate-400">Ποιο κατάστημα επιλέγεται για κάθε προϊόν.</p>
                </div>
              </div>

              {basket.length === 0 && (
                <div className="mt-6 rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-6 text-center text-sm font-semibold text-slate-500">
                  {EMPTY_TEXT}
                </div>
              )}
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="text-right">
                          <div className="text-sm font-black text-slate-800">{item.subtotal.toFixed(2)}€</div>
